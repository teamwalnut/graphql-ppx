...
let scalars =
  (module struct
    ...
    let pp formatter (obj : scalars) =
      Format.fprintf formatter "{ string = %a ; int = %a }"
...
let my_query =
  (module struct
    ...
    let pp formatter (obj : qt) =
      Format.fprintf formatter "< variousScalars = @[%a@] >"
...
let decodes_record_in_external_fragment () =
  Alcotest.check external_fragment_query "query result equality"
    ({| {"variousScalars": {"string": "a string", "int": 123}} |}
...
let decodes_record_in_external_fragment_on_union_selections () =
  Alcotest.check union_external_fragment_query "query result equality"
    ({| {"dogOrHuman": {"__typename": "Dog", "name": "name", "barkVolume": 123.0}} |}

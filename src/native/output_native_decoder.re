    ...
    Ast_helper.(
      Typ.variant(
        List.map(
          ({poly_variant_name, _}) =>            
            Rtag(
              {txt: Compat.capitalize_ascii(poly_variant_name), loc},
              [],
              false,
              [
                {
...
and generate_poly_variant_interface = (config, loc, name, base, fragments) => {
  let map_fallback_case = ({poly_variant_name, res_structure: inner, _}) => {
    open Ast_helper;
    let name_pattern = Pat.any();
    let variant =
      Exp.variant(poly_variant_name, Some(generate_decoder(config, inner)));
    Exp.case(name_pattern, variant);
  };

  let map_case = ({poly_variant_name, res_structure: inner, _}) => {
    open Ast_helper;
    let name_pattern =
      Pat.constant(Pconst_string(poly_variant_name, None));
    let variant =
      Exp.variant(poly_variant_name, Some(generate_decoder(config, inner)));
    Exp.case(name_pattern, variant);
  };
  ...
  let {poly_variant_name: base_name, res_structure: base_decoder, _} = base;
  let fallback_case_ty = map_case_ty(({txt: base_name, loc}, base_decoder));

  let fragment_case_tys =
    List.map(
      map_case_ty,
      fragments |> List.map(({poly_variant_name, res_structure, _}) => ({txt: poly_variant_name, loc}, res_structure)),
    );
  let interface_ty =
    Ast_helper.(
...
and generate_poly_variant_union =
  ...
  let fragment_cases =
    Ast_helper.(
      fragments
      |> List.map(({poly_variant_name, res_structure: inner, _}) => {
           let name_pattern =
             Pat.constant(Pconst_string(poly_variant_name, None));
           let variant =
             Ast_helper.(

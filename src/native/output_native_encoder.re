...
let rec parser_for_type = (schema, loc, type_ref) => {
  ...
  | Ntr_list(x) =>
    ...
    [@metaloc loc]
    [%expr (v => `List(Array.map([%e child_parser], v) |> Array.to_list))];
  | Ntr_nullable(x) =>
    let child_parser = parser_for_type(schema, loc, x);
    let loc = conv_loc(loc);
    [@metaloc loc]
  ...
  | Ntr_named(type_name) =>
    ...
    | Some(Scalar(_)) =>
      %expr
      (v => v)
    | Some(ty) => function_name_string(ty) |> ident_from_string(loc)
    };
  };
};
...
let json_of_fields = (schema, loc, expr, fields) => {
  let field_array_exprs =
    ...
    |> List.map(({am_name, am_arg_type, _}) => {
         ...
         [%expr
           (
             [%e
               Ast_helper.Exp.constant(Pconst_string(am_name, None))
             ],
             [%e parser](
  ...
  let loc = conv_loc(loc);
  [@metaloc loc] [%expr `Assoc(Array.to_list([%e field_array]))];
};

...
let map_next_token parser f =
  match next parser with Ok item -> Ok (map f item) | e -> e

let replace_next_token parser t =
  match next parser with Ok item -> Ok (replace item t) | Error e -> Error e

let map_next_token_with_error parser f =
  match next parser with Ok item -> Error (map f item) | Error e -> Error e
...
let parse_variable_literal parser =
  ...
  |> map (fun (start_pos, { item; span = _, end_pos }) ->
...
let rec parse_value_literal is_const parser =
  ...
  | { item = Graphql_lexer.Dollar; _ } when not is_const ->
...
and parse_list_literal is_const parser =
  delimited_list parser Graphql_lexer.Bracket_open
...
and parse_object_literal is_const parser =
  delimited_list parser Graphql_lexer.Curly_open

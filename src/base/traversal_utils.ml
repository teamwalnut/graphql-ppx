...
type ctx = {
  ...
  warnings : (Result_structure.loc * string) list ref;
...
module type VisitorSig = sig
  ...
  val enter_variable_definition :
    t -> ctx -> string spanning * variable_definition -> unit

  val exit_variable_definition :
    t -> ctx -> string spanning * variable_definition -> unit
  ...
  val enter_argument :
    t ->
    ctx ->
    string spanning * Schema.type_ref option * input_value spanning ->
    unit
  ...
  val enter_list_value : t -> ctx -> input_value spanning list spanning -> unit
  val exit_list_value : t -> ctx -> input_value spanning list spanning -> unit

  val enter_object_value :
    t -> ctx -> (string spanning * input_value spanning) list spanning -> unit

  val exit_object_value :
    t -> ctx -> (string spanning * input_value spanning) list spanning -> unit

  val enter_object_field :
    t -> ctx -> string spanning * input_value spanning -> unit

  val exit_object_field :
    t -> ctx -> string spanning * input_value spanning -> unit
end
...
module Context = struct
  ...
  let push_parent_type ctx =
    let top = match ctx.type_stack with t :: _ -> t | [] -> None in
    { ctx with parent_type_stack = top :: ctx.parent_type_stack }

  let parent_type ctx =
    match ctx.parent_type_stack with t :: _ -> t | [] -> None

  let current_input_type_literal ctx =
    match ctx.input_type_literal_stack with t :: _ -> t | [] -> None
...
module Visitor (V : VisitorSig) = struct
  ...
  let rec visit_input_value self ctx (arg_type : Schema.type_ref option) value =
  ...
  and visit_field self ctx field =
    ...
    let () =
      match field.item.fd_selection_set with
      | None -> ()

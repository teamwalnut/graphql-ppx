...
let rec unify_type ~has_decoder error_marker as_record existing_record
  ...
  | Ntr_named n -> (
      ...
      | Some (Scalar { sm_name }) -> (
          ...
          | other -> raise other)
      | Some (Object _ as ty) ->
          unify_selection_set error_marker as_record existing_record config span
            ty selection_set
      | Some (Enum enum_meta) ->
          Res_poly_enum
            { loc = config.map_loc span; enum_meta; omit_future_value }
      ...
      | Some (Union um) ->
          unify_union error_marker config span um omit_future_value
            selection_set)
          ...
          Res_object
            {
              ...
              fields =
                List.map
                  (unify_selection ~base_type:interface_meta.im_name
                     error_marker config ty)
                  selection;
              type_name = None;
  ...
  | Some selection_set ->
      ...
      let generate_case
        ...
        | Some if_type_condition ->
            ...
            in
            let is_record =
              has_directive ~prepend:true "Record" if_directives
            in
      ...
      Res_poly_variant_union
        {
          ...
          exhaustive =
            (if covered_cases = possible_cases then Exhaustive
            else Nonexhaustive);
          omit_future_value;
        }
                ...
                |> List.map (fun selection ->
                       ...
                       | Field { item; _ } -> (
                           ...
                           | Some field_meta ->
                               ...
                               in
                               ( key,
                                 unify_type ~has_decoder:false error_marker
                                   false None false config span inner_type
...
and unify_field error_marker config field_span ty =
  ...
  | `None ->
      Fr_named_field
        { name = key; loc_key; loc; type_ = parser_expr; arguments }
...
let get_template_tag query_config =
  ...
  | _ -> (
      ...
      | Some tag, Some location, None ->
          (Some tag, Some location, Some "default")

type type_ref = Named of string | NonNull of type_ref | List of type_ref
...
type scalar_meta = { sm_name : string; sm_description : string option }
...
let mutation_type s =
  match s.meta.sm_mutation_type with
  | Some n -> Some (Hashtbl.find s.type_map n)
  | None -> None

let subscription_type s =
  match s.meta.sm_subscription_type with
  | Some n -> Some (Hashtbl.find s.type_map n)
  | None -> None
...
let is_type_default ty =
  ...
  | "ID" | "String" | "Int" | "Float" | "Boolean" -> true
  | _ -> false
...
let all_enums schema =
  Hashtbl.fold
    (fun _ v acc -> match v with Enum e -> e :: acc | _ -> acc)
    schema.type_map []

...
let ser_optional_list = serializer =>
  fun
  | None
  | Some({item: [], _}) => `List([])
...
let ser_name = ({item: name, _}) =>
  `Assoc([("kind", `String("Name")), ("value", `String(name))]);
...
let ser_variable_definition = ((name, def)) =>
  `Assoc([
    ...
    (
      "variable",
      `Assoc([("kind", `String("Variable")), ("name", ser_name(name))]),
    ),
    ("type", ser_type(def.vd_type)),
    ("defaultValue", ser_optional(ser_value, def.vd_default_value)),
  ]);

let ser_variable_definitions = defs =>
  ser_optional_list(ser_variable_definition, defs);
...
let rec ser_selection_set = sset =>
  `Assoc([
...
and ser_selection =
  ...
  | Field({item, _}) =>
    `Assoc([
      ...
      (
        "selectionSet",
        ser_optional(ser_selection_set, item.fd_selection_set),
      ),
    ])
...
let def_end =
  fun
  | Operation({span: (_, e), _})
  | Fragment({span: (_, e), _}) => e.index;

let document_end =
  List.fold_left((end_, def) => max(end_, def_end(def)), 0);

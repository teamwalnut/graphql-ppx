       ...
       | Some { span = start_pos, _; _ } ->
         ...
         |> flat_map
              ...
              | { item = Graphql_lexer.Exclamation_mark; span = _, end_pos } ->
                next parser
                |> replace
                     (start_end start_pos end_pos (Tr_non_null_list inner_type))
              | _ ->
                Ok (start_end start_pos bracket_end_pos (Tr_list inner_type)))
       | None ->
         expect_name parser
         |> flat_map (fun name ->
              match peek parser with
...
let parse_fragment_definition parser =
  ...
  |> flat_map (fun (start_pos, name) ->
       ...
       | { item = "on"; _ } as span ->
         Error
           (Source_pos.replace span (Unexpected_token (Graphql_lexer.Name "on")))
       | _ -> Ok (start_pos, name))
